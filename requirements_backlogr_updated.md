...
- Store common fields locally so the app stays fast and stable:
  - title, summary/description, cover image URL, release date, platforms, genres, developer, publisher
...
- **Rating source of truth:** rating is stored on **GameLog**, not on **Review**.
...
- **User**
  ...
  - FirstName *(optional)*
  - LastName *(optional)*
  - AvatarUrl *(optional, URL only for MVP)*
  - Bio *(optional)*
  - CreatedAt
...
- **Game**
  - **GameId** (PK)
  - IgdbId *(unique, optional until imported)*
  - Title
  - Slug *(optional)*
  - Summary *(optional)*
  ...
  - Developer *(optional)*
  - Publisher *(optional)*
  - Platforms *(stored as string for MVP)*
  - Genres *(stored as string for MVP)*
  - CreatedAt
  - UpdatedAt
...
- **Review**
  ...
  - CreatedAt
  - UpdatedAt
  - **Constraint:** unique `(UserId, GameId)` so a user has one review per game
  - **Note:** Review does **not** store rating; `GameLog.Rating` is the single source of truth
...
- Integration-style tests:
  - WebApplicationFactory for core happy-path endpoint flows
